...
void BFS_traverse(PBFS_QUEUE pbfs_queue)
{
   while(!BFS_queue_empty(pbfs_queue)) {
        struct bt_node *parent = BFS_dequeue(pbfs_queue);
        if(parent != NULL) {
            printf(" %d", parent->v);
        }
        if (parent->lt) {
            BFS_enqueue(pbfs_queue, parent->lt);
        }

        if (parent->rt) {
            BFS_enqueue(pbfs_queue, parent->rt);
        }
        BFS_traverse(pbfs_queue);
   }
...
int main (int argc, char * argv[])
{
    ...
    bfs_queue.H = bfs_queue.T = bfs_queue.Q;
    BFS_enqueue(&bfs_queue, R);
    printf("\nBFS (Level Order)   :");
    BFS_traverse(&bfs_queue);
    printf("\n");
    return 0;
}
